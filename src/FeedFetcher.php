...
{
    /**
     * HTTP statuses that mean "the server couldn't answer right now", not
     * "this feed is gone" — worth another attempt. 404 is deliberately not in
     * here: for virtually every host it's a permanent answer, and retrying it
     * only delays reporting a genuinely dead feed. YouTube is the documented
     * exception (see isTransientFailure()).
     */
    private const RETRYABLE_STATUS = [408, 425, 429, 500, 502, 503, 504];

    /** Delay before retry N, multiplied by the attempt number for a simple linear backoff. */
    private const RETRY_BASE_DELAY_MICROSECONDS = 400_000;

    /**
     * Whether a failed fetch looks temporary, so callers can retry it rather
     * than treating the feed as broken.
     *
     * YouTube's feed endpoint (its own "YouTube RSS Feeds server") regularly
     * answers 404 — in bursts lasting minutes — for channel ids that are
     * perfectly valid and serve 200 again immediately afterwards, so a 404
     * from there says nothing about whether the feed exists. Since a URL under
     * youtube.com/feeds/videos.xml is only ever generated by YouTubeResolver
     * from a channel that resolved successfully, treating its 404s as
     * transient can't mask a user's typo the way it would elsewhere.
     */
    public static function isTransientFailure(FeedFetchResult $result, string $url): bool
    {
        if ($result->error !== null) {
            return true;
        }
        if (in_array($result->httpCode, self::RETRYABLE_STATUS, true)) {
            return true;
        }
        return $result->httpCode === 404 && self::isYouTubeFeedUrl($url);
    }

    private static function isYouTubeFeedUrl(string $url): bool
    {
        $parts = parse_url($url);
        $host = strtolower($parts['host'] ?? '');
        return in_array($host, ['youtube.com', 'www.youtube.com', 'm.youtube.com'], true)
            && str_starts_with($parts['path'] ?? '', '/feeds/videos.xml');
    }

    /** Fetches one feed, retrying transient failures up to FETCH_RETRY_ATTEMPTS times in total. */
    public static function fetchRaw(string $url, ?string $etag, ?string $lastModified): FeedFetchResult
    {
        for ($attempt = 1; ; $attempt++) {
            $result = self::fetchRawOnce($url, $etag, $lastModified);
            if ($attempt >= FETCH_RETRY_ATTEMPTS || !self::isTransientFailure($result, $url)) {
                return $result;
            }
    ...
    private static function fetchRawOnce(string $url, ?string $etag, ?string $lastModified): FeedFetchResult
    {
     ...
     *
     * Transient failures (see isTransientFailure()) are retried, up to
     * FETCH_RETRY_ATTEMPTS attempts in total. Each retry round re-fetches only
     * the feeds that failed, again concurrently, so the added wall-clock cost
     * is one extra round trip per round rather than one per failed feed.
     *
    ...
    public static function fetchManyRaw(array $requests, int $concurrency): array
    {
        ...
        for ($attempt = 1; $pending !== []; $attempt++) {
            foreach (self::fetchManyRawOnce($pending, $concurrency) as $key => $result) {
                $results[$key] = $result;
            }
            ...
            $pending = array_filter(
                $pending,
                fn ($key) => self::isTransientFailure($results[$key], $pending[$key]['url']),
     ...
     * @return array<int|string, FeedFetchResult> keyed the same way as $requests
     */
    private static function fetchManyRawOnce(array $requests, int $concurrency): array
    {
        $results = [];
        if ($requests === []) {
            return $results;
