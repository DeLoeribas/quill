...
- **Saved searches**: while viewing search results, click "Save" next to the search box to name and save that query. Saved searches appear in the sidebar in their own collapsible "Saved Searches" group (below "Saved", above your folders, same collapse/remember behavior as a folder or the Tags section) showing a live count of how many items currently match; click one to view its results, or hover it and click the ✕ that appears to delete it (only removes the saved shortcut, not any items).
