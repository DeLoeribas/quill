...
Full reference documentation. For a quick overview, see [README.md](README.md).
...
- Personal notes and tags on items — a "Notes" view appears once you have at least one item with a note attached, showing only those across every feed; tags get their own browsable, collapsible sidebar section (with autocomplete when adding one) and can be searched via `tag=`
...
- Item list requests don't carry each item's full summary/content (often tens of KB of raw article HTML per item) — the reading pane fetches that on demand for whichever single item you open, keeping list loads fast even on feeds with long articles
- Items are prefetched as you scroll: once a row has been on screen for ~250ms, its summary is fetched in the background (at most 3 requests at once; rows scrolled away from before their turn are dropped from the queue), followed by the images the reading pane will show for it (up to 6 per item) to warm the browser cache — so opening a visible item usually shows its content and images instantly instead of "Loading…". Nothing new starts while the tab is hidden
...
- **Saved searches**: while viewing search results, click "Save" next to the search box to name and save that query. Saved searches appear in the sidebar in their own collapsible "Saved Searches" group (below "Saved" and "Notes" when either is showing, above the Tags section and your folders, same collapse/remember behavior as a folder) showing a live count of how many items currently match; click one to view its results, or hover it and click the ✕ that appears to delete it (only removes the saved shortcut, not any items).
- **Personal notes**: in the reading pane, click "+ Add a note" to attach a private note to that item (not shared anywhere, not part of the article itself) — "Edit note"/"Delete note" appear once one exists. Notes are included in the header search (title + summary + note + feed name) and drive the sidebar's "Notes" view, which shows only items with a note attached across every feed, same pattern as "Saved".
- **Tags**: also in the reading pane, type into the tag field to add one or more tags to an item — matching existing tags are suggested as you type (autocomplete), and clicking an existing tag's ✕ removes it. Every tag in use across all your items appears in its own collapsible "Tags" sidebar section (count per tag, click a tag to view only items carrying it); tags are normalized to trimmed lowercase with duplicates dropped.
